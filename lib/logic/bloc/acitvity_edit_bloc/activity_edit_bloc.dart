...
class ActivityEditBloc extends Bloc<ActivityEditEvent, ActivityEditState> {
  ...
  ActivityEditBloc(this.givenActivity)
  ...
  _changedTitle(
      ActivityChangedTitleEvent event, Emitter<ActivityEditState> emit) {
    final activity = (state as ActivityEditChangedState).activity;
    emit(ActivityEditChangedState(
        activity: activity.copyWith(title: event.title)));
  }

  _changedDescription(
      ActivityChangedDescriptionEvent event, Emitter<ActivityEditState> emit) {
    final activity = (state as ActivityEditChangedState).activity;
    emit(ActivityEditChangedState(
        activity: activity.copyWith(description: event.description)));
  }

  _changedTime(
      ActivityChangedTimeEvent event, Emitter<ActivityEditState> emit) {
    final activity = (state as ActivityEditChangedState).activity;
    emit(ActivityEditChangedState(
        activity: activity.copyWith(time: event.time)));
  }
}
